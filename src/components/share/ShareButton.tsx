...
// Individual share button for each platform

'use client'

import { cn } from '@/lib/utils'

export type SharePlatform = 'twitter' | 'facebook' | 'whatsapp' | 'linkedin' | 'telegram' | 'email'
...
export function ShareButton({
  ...
  const sizeClasses = {
    sm: {
      ...
      pill: 'h-8 px-3 text-xs gap-1.5',
      iconSize: 'w-4 h-4',
    },
    md: {
      icon: 'w-10 h-10',
      button: 'h-10 px-4 text-sm gap-2',
      pill: 'h-10 px-4 text-sm gap-2',
      iconSize: 'w-5 h-5',
    },
    lg: {
      icon: 'w-12 h-12',
      button: 'h-12 px-5 text-base gap-2',
      pill: 'h-12 px-5 text-base gap-2',
      iconSize: 'w-6 h-6',
    },
  }
  ...
  if (variant === 'icon') {
    return (
      <button
        ...
        aria-label={`${config.name}'da paylaş`}
        title={config.name}
      >
        <span className={sizeClasses[size].iconSize}>{config.icon}</span>
      </button>
    )
  }
